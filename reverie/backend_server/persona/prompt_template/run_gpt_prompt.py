...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        if USE_REGEX:
            return re.search(
                r"^\d{1,2}(?::\d{2})?[aA][mM]", gpt_response.strip()
            ).group()[0]
        else:
...
def run_gpt_prompt_daily_plan(persona, wake_up_hour, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        if USE_REGEX:
            return re.findall(
                r"\d+\)\s+(.*?)(?=, \d+\)|\.|$)", " ".join(gpt_response.split("\n"))
            )
        else:
            cr = []
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ...
        for count, i in enumerate(temp_clean_response):
            ...
            if len(k) < 2:
                print(
                    f"Warning: Unexpected string structure in '{i}'. Missing '(duration in minutes:' delimiter."
                )
                continue
            ...
            try:
                ...
                print(
                    f"Error: Failed to convert '{k[1].split(',')[0].strip()}' to integer."
                )
                duration = 0
                continue
