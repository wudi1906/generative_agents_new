...
def run_gpt_prompt_generate_hourly_schedule(persona, 
  ...
  def create_prompt_input(persona, 
    ...
    prior_schedule_inst = "According to today schedule above,replace the [Fill in] with actions in the Hourly schedule:\n"
    ...
    prompt_input = []
    prompt_input += [schedule_format]#输出格式
    prompt_input += [persona.scratch.get_str_iss()]#人物介绍
    prompt_input += [intermission_str]#今天大致日程
    prompt_input += [prior_schedule_inst]#如何填写的说明
    prompt_input += [prior_schedule]#6点前的sample时间表
    prompt_input += [prompt_ending]#结尾的接续
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    print ("TOODOOOOOO---crashly \ngpt_response start--->")
    print (gpt_response)
    print ("gpt_response end--->")
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False): 
  ...
  print("---->run_gpt_prompt_act_obj_desc v3_ChatGPT")
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/generate_obj_event_v1.txt" #bed被睡的魔板
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved,test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v2/decide_to_talk_v2.txt" #v1的step-by-step会误导
...
def run_gpt_prompt_agent_chat_summarize_relationship(persona, target_persona, statements, test_input=None, verbose=False): 
  ...
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = 'close relationship'
  special_instruction = 'The output should be a string that responds to the question.'
  fail_safe = get_fail_safe()
  #output = ChatGPT_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
  #                                        __chat_func_validate, __chat_func_clean_up, True)
...
def run_gpt_generate_safety_score(persona, comment, test_input=None, verbose=False): 
  ...
  print ("---->run_gpt_generate_safety_score")
  prompt = generate_prompt(prompt_input, prompt_template)
  fail_safe = get_fail_safe() 
...
def run_gpt_generate_iterative_chat_utt(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None, verbose=False): 
  ...
  def __chat_func_validate(gpt_response, prompt=""): 
    try: 
      ...
      print ("---->extract_first_json_dict:TRUE")
      ##todo wether talk is end
      return True
    except:
      print ("---->extract_first_json_dict:FALSE")
      #todo stop talk
      return False 
  ...
  print("---->run_gpt_generate_iterative_chat_utt:init")
  print(curr_context)
  print(curr_chat)  
  ...
  print ("---->run_gpt_generate_iterative_chat_utt:output agent response")
  print (output)
