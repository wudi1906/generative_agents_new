...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "gpt-4o-mini", "max_tokens": 5, 
...
# 0302 - add
    ...
    try:
        ...
        for count, i in enumerate(_cr): 
            try:
                ...
                print(f"Warning: Failed to parse task: {i}, error: {e}")
                continue
                
        if not cr:  # if no tasks were successfully parsed
            print("No valid tasks found, using default")
            total_expected_min = int(prompt.split("(total duration in minutes")[-1].split("):")[0].strip())
            return [["default task", total_expected_min]]
            
        return cr
    except Exception as e:
        print(f"Error in task parsing: {e}")
        total_expected_min = int(prompt.split("(total duration in minutes")[-1].split("):")[0].strip())
        return [["default task", total_expected_min]]
  ...
  gpt_param = {"engine": "gpt-4o-mini", "max_tokens": 1000, 
             "temperature": 0, "top_p": 1, "stream": False,
             "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved,test_input=None, 
  ...
  gpt_param = {"engine": "gpt-4o-mini", "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
...
def run_gpt_prompt_keyword_to_thoughts(persona, keyword, concept_summary, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "gpt-4o-mini", "max_tokens": 40, 
               "temperature": 0.7, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
