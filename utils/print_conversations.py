...
def get_unique_conversations(simulation_name):
  step_folder = os.path.join(PROJECT_ROOT, "environment", "frontend_server", "storage")
...
def write_conversations_to_file(conversations, simulation_name):
  output_directory = os.path.join(PROJECT_ROOT, "logs", "conversations")
...
if __name__ == "__main__":
  ...
  if unique_conversations:
    ...
    print(
      f"Unique conversations written to logs/conversations/{simulation_name}_highlights.json"
    )
  else:
